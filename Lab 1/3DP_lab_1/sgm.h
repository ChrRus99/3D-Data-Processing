...
namespace sgm
{
  ...
  public:
    ...
    // Computes the improved right-to-left disparity map
  ...
  private:
      ...
      // Computes a column of the integration matrix E(cur_p, d) for a pixel p, for all possible disparities d in [0, disparity_range)
      void compute_path_cost(int direction_y, int direction_x, int cur_y, int cur_x, int cur_path);
      // Integrate path costs E(p, d), for each pixel p in the image, along all directions 
      void aggregation();
