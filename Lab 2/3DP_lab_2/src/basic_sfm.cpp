...
struct ReprojectionError
{
  ...
  ReprojectionError(double observed_x, double observed_y, double focal_length)
  ...
  bool operator()(const T* const camera_pose, const T* const point, T* residuals) const {
    // Define a 2D point (homogeneous coordinates)
    T p[3];

    // Rotate the point using the camera rotation: camera [0,1,2] angle-axis rotation
    ceres::AngleAxisRotatePoint(camera_pose, point, p);

    // Add translation to the rotated point: camera[3,4,5] translation
    p[0] += camera_pose[3];
    p[1] += camera_pose[4];
    p[2] += camera_pose[5];

    // Dehomogenization of the 2D point
    T xp = p[0] / p[2];
    T yp = p[1] / p[2];

    // Compute final projected point position
    T predicted_x = focal_length * xp;
    T predicted_y = focal_length * yp;
  ...
  static ceres::CostFunction* Create(double observed_x, double observed_y, double focal_length=1.0) { // canonical camera (f=1.0) by default
    return new ceres::AutoDiffCostFunction<ReprojectionError, 2, 6, 3>(
      new ReprojectionError(observed_x, observed_y, focal_length)
    );
  }
...
{
  // Store the parameters of the previous reconstruction
  vector<double> prev_parameters;

  for(const auto& param : parameters_) {
    prev_parameters.push_back(param);
  } 

  // Store previous bounding box size
  double prev_bbox_volume = -1.0;
  ...
  const double RANSAC_THRESHOLD = 0.0015;

  // Estimate the Essential matrix E
  cv::Mat E = cv::findEssentialMat(points0, points1, intrinsics_matrix, cv::RANSAC, RANSAC_PROB, RANSAC_THRESHOLD, inlier_mask_E);
  
  // Estimate the Homograph matrix H
  ...
  if(num_inliers_E <= num_inliers_H) {
    return false; // Try a new seed pair
  }

  // Recover from E the initial rigid body transformation R,t between seed_pair_idx0 and seed_pair_idx1 cameras
  cv::Mat R, t;
  ...
  // (i.e., if the translation along x is greater than the translation along z or 
  //        if the translation along y is greater than the translation along y)
  double translation_x = std::abs(t.at<double>(0));
  double translation_y = std::abs(t.at<double>(1));
  double translation_z = std::abs(t.at<double>(2));

  //if ((translation_x < translation_z)) {
  if((translation_x < translation_z) || (translation_y < translation_z)) {
      return false; // Try a new seed pair
  }
  
  // Store the initial rigid body transformation R,t between seed_pair_idx0 and seed_pair_idx1
  R.copyTo(init_r_mat);
  t.copyTo(init_t_vec);
  ...
  for(int iter = 1; iter < num_cam_poses_ - 1; iter++ )
  {
      ...
      if( cam_pose_optim_iter_[cam_idx] > 0 )
      {
        for( auto const& co_iter : cam_observation_[cam_idx] )
        {
              ...
              cam_observation_[new_cam_pose_idx].find(pt_idx ) != cam_observation_[new_cam_pose_idx].end() )
          {
            ...
            // cam0_data[0], cam0_data[1], cam0_data[2] hold the axis-angle representation for the rotation of the
            ...
            // Extract and store 2D image points from the new and the current camera poses
            cv::Point2d pt_new_cam = cv::Point2d(
              observations_[cam_observation_[new_cam_pose_idx][pt_idx] * 2],
              observations_[cam_observation_[new_cam_pose_idx][pt_idx] * 2 + 1]
            );
            points0.push_back(pt_new_cam);

            cv::Point2d pt_curr_cam = cv::Point2d(
              observations_[cam_observation_[cam_idx][pt_idx] * 2],
              observations_[cam_observation_[cam_idx][pt_idx] * 2 + 1]
            );
            points1.push_back(pt_curr_cam);

            // Axis-angle representation of the new camera pose
            cv::Vec3d r0(cam0_data[0], cam0_data[1], cam0_data[2]); 
            // Convert axis-angle to rotation matrix
            cv::Matx33d R0;
            cv::Rodrigues(r0, R0); 
            // Translation vector of the new camera pose
            cv::Vec3d t0(cam0_data[3], cam0_data[4], cam0_data[5]);
            // Build the projection matrix of the new camera pose
            proj_mat0 = intrinsics_matrix * cv::Matx34d(R0(0,0), R0(0,1), R0(0,2), t0(0),
                                                        R0(1,0), R0(1,1), R0(1,2), t0(1),
                                                        R0(2,0), R0(2,1), R0(2,2), t0(2));

            // Axis-angle representation of the current camera pose
            cv::Vec3d r1(cam1_data[0], cam1_data[1], cam1_data[2]);
            // Convert axis-angle to rotation matrix
            cv::Matx33d R1;
            cv::Rodrigues(r1, R1);
            // Translation vector of the current camera pose
            cv::Vec3d t1(cam1_data[3], cam1_data[4], cam1_data[5]);
            // Build the projection matrix of the current camera pose
            proj_mat1 = intrinsics_matrix * cv::Matx34d(R1(0,0), R1(0,1), R1(0,2), t1(0),
                                                        R1(1,0), R1(1,1), R1(1,2), t1(1),
                                                        R1(2,0), R1(2,1), R1(2,2), t1(2));

            // Triangulate the 3D point using the observations from both camera poses
            cv::triangulatePoints(proj_mat0, proj_mat1, points0, points1, hpoints4D);
            
            // Check the cheirality constraint for both cameras
            double depth = hpoints4D(2) / hpoints4D(3); // dehomogenized z coordinate = depth of the triangulated point
            
            //if(checkCheiralityConstraint(new_cam_pose_idx, pt_idx) && checkCheiralityConstraint(cam_idx, pt_idx)) { // NON FUNZIONA !!!         
            if(depth > 0) { // if depth > 0 it means that the point is in front of both cameras => satisfies the cheirality constraint
              // Increment the counter of triangulated points
              n_new_pts++;
                  
              // Mark the point as optimized
              pts_optim_iter_[pt_idx] = 1;

              // Dehomogenization of the triangulated point
              double *pt = pointBlockPtr(pt_idx);
              pt[0] = hpoints4D(0) / hpoints4D(3);
              pt[1] = hpoints4D(1) / hpoints4D(3);
              pt[2] = hpoints4D(2) / hpoints4D(3);
            }
    ...
    const double MAX_VOLUME_CHANGE = 25;  // percentage range (0,1]

    // Compute the current bounding box volume
    double bbox_volume = (vol_max.x() - vol_min.x()) * (vol_max.y() - vol_min.y()) * (vol_max.z() - vol_min.z());

    // Calculate the percentage of change in volume
    double volume_change_percentage = abs((bbox_volume - prev_bbox_volume) / prev_bbox_volume);

    // cout << "BOUNDING BOX VOLUME PREVIOUS/CURRENT: ---> [" << prev_bbox_volume << ", " << bbox_volume << "] ---> GROWS: [" << volume_change_percentage << "]" << endl; 

    // Check if the dimensions of the bounding box change too much
    if((prev_bbox_volume != -1.0) && (volume_change_percentage > MAX_VOLUME_CHANGE)) {
        return false; // Reset the reconstruction and start from scratch
    }
    
    // Update previous bounding box dimensions
    prev_bbox_volume = bbox_volume;


    /*
    // ALTERNATIVE METHOD, EQUIVALENT RESULTS AS THE ONE ABOVE
    // Define a threshold for detecting divergence
    const double CAMERA_THRESHOLD = 0.40;
    const double POINT_THRESHOLD = 0.40;
    const double TOLERANCE_RATIO = 0.6;

    // Count the number of cameras exceeding the threshold
    int count_camera_div = 0;

    for (int i_c = 0; i_c < num_cam_poses_; i_c++) {  // for each camera
      //cout << "-----> " << cam_pose_optim_iter_[i_c] << endl;
      for(int p = 0; p < camera_block_size_; p++) {   // for each camera parameter
        //cout << "CAMERA [" << i_c << "] PARAMETERS PREV/CURR: ---> [" << prev_parameters[i_c * p] << ", " << parameters_[i_c * p] << "]" << endl;

        if (std::fabs(parameters_[i_c * p] - prev_parameters[i_c * p]) > CAMERA_THRESHOLD) {
          //cout << "---> CAMERA DIVERGE" << endl;
    ...
    // Count the number of points exceeding the threshold
    int count_point_div = 0;

    for (int i_p = num_cam_poses_; i_p < num_cam_poses_ + num_points_; i_p++) { // for each point
      //cout << "-----> " << pts_optim_iter_[i_p] << endl;
      for(int p = 0; p < point_block_size_; p++) {                              // for each point parameter
        //cout << "POINT [" << i_p << "] PARAMETERS PREV/CURR: ---> [" << prev_parameters[i_p * p] << ", " << parameters_[i_p * p] << "]" << endl;

        if (std::fabs(parameters_[i_p * p] - prev_parameters[i_p * p]) > POINT_THRESHOLD) {
          //cout << "---> POINT DIVERGE" << endl;
    ...
    // Check divergence 
    // Diverge if the number of cameras diverging or the number of points diverging is greather than TOLERANCE_RATIOs
    if (count_camera_div > TOLERANCE_RATIO*num_cam_poses_ || count_point_div > TOLERANCE_RATIO*num_points_) {
      //std::cout << "BAD RECONSTRUCTION, RESTART FROM SCRATCH WITH A NEW PAIR OF SEED IMAGES" << std::endl;
      return false; // Reset reconstruction and restart from scratch with a new pair of seed images
    }

    // Update the previous parameters with the current parameters for the next reconstruction
...
void BasicSfM::bundleAdjustmentIter( int new_cam_idx )
{
  ...
  while( keep_optimize )
  {
    ...
    for( int i_obs = 0; i_obs < num_observations_; i_obs++ )
    {
      //.. check if this observation has been already registered (both checking camera pose and point pose)
      if( cam_pose_optim_iter_[cam_pose_index_[i_obs]] > 0 && pts_optim_iter_[point_index_[i_obs]] > 0 )
      {
        ...
        // Set the loss function (Cauchy loss with parameter 2 * max_reproj_err_)
        ceres::LossFunction* loss_function = new ceres::CauchyLoss(2 * max_reproj_err_);

        // Retrieve the pointers to camera parameters and to points from parameters_ vector
        //double* camera_ptr = parameters_.data() + camera_block_size_ * cam_pose_index_[i_obs];
        //double* point_ptr = parameters_.data() + camera_block_size_ * num_cam_poses_ + point_block_size_ * point_index_[i_obs];
